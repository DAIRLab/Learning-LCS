...
class LCS_OC:
    def __init__(self, A, B, C, D, E, F):
        ...
        # loss function
        lcp_loss = dot(self.D @ x + self.E @ u + self.F @ lam, lam)

        # constraints
        dis_cstr = self.D @ x + self.E @ u + self.F @ lam
        lam_cstr = lam
    ...
    def solve(self, state_int, horizon, mat_Q, mat_R):
        ...
        for k in range(horizon):
            ...
            g += [self.D @ Xk + self.E @ Uk + self.F @ Lamk]
            lbg += self.n_lam * [0]
            ubg += self.n_lam * [inf]

            g += [casadi.dot(self.D @ Xk + self.E @ Uk + self.F @ Lamk, Lamk)]
            lbg += [0]
            ubg += [0]
...
class LCS_VN:
    ...
    def diff(self, gamma=1e-2, epsilon=1., F_ref=0., w_F=0., C_ref=0., w_C=0.):
        ...
        self.pred_qpSolver = qpsol('pred_qpSolver', 'qpoases', pred_quadprog, opts)
