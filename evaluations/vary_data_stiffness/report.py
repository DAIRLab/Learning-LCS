...
print('-----')
meta_para_list = np.array(load['meta_para_list'][0:-1])
meta_para_list2 = np.array(load2['meta_para_list'])
print('list of state dim:', meta_para_list)
print('list of state dim:', meta_para_list2)
print('-----')
...
results_list = load['meta_para_trials'][0:-1]
...
for i_meta_para in range(len(results_list)):
    ...
    for i_trial in range(len(trials_list)):
        ...
        training_mode_count += [trials_list[i_trial]['training_data_mode_percentage'] * (2 ** 10.0)]
    ...
    for i_trial in range(len(trials_list2)):
        ...
        training_mode_count += [trials_list2[i_trial]['training_data_mode_percentage'] * (2 ** 10.0)]
...
label = ['1', '$10^{-1}$', '$10^{-2}$', '$10^{-3}$', '$10^{-4}$']
plt.xticks(x,labels=label)
plt.xlabel('Data stiffness $\sigma_{min}(F+F^T)$', labelpad=15)
