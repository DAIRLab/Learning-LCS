...
# storage parameter
meta_para_list = [1, 4, 8, 12, 16, 20]
meta_para_trials = []
for meta_para in meta_para_list:

    # establish the PN learner (prediction-based method)
    pn_learner = lcs_learning.LCS_PN(n_state=n_state, n_control=n_control, n_lam=meta_para, F_stiffness=F_stiffness)
    ...
    vn_learner = lcs_learning.LCS_VN(n_state=n_state, n_control=n_control, n_lam=meta_para, F_stiffness=F_stiffness)
    ...
    tester = lcs_learning.LCS_PN(n_state=n_state, n_control=n_control, n_lam=meta_para, F_stiffness=F_stiffness)
    ...
    for n_trial in range(max_trial):
        ...
        lcs_mats = lcs_learning.gen_lcs(n_state, n_control, meta_para, gen_stiffness)
        ...
        for iter in range(max_iter):
            ...
            if iter % 100 == 0:
                print('dimension of lam:', meta_para,
                      '|| trial #:', n_trial,
                      '|| iter', iter,
