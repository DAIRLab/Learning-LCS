...
# storage parameter
meta_para_list = [0.5, 0.1, 0.01, 0.001, 0.0001]
meta_para_trials = []
for meta_para in meta_para_list:

    # establish the PN learner (prediction-based method)
    pn_learner = lcs_learning.LCS_PN(n_state=n_state, n_control=n_control, n_lam=n_lam, F_stiffness=F_stiffness)
    ...
    vn_learner.diff(gamma=meta_para, epsilon=epsilon, w_C=1e-6, C_ref=0, w_F=0e-6, F_ref=0)
    vn_optimizier = opt.Adam()
    vn_optimizier.learning_rate = vn_learning_rate

    # establish the testing object
    tester = lcs_learning.LCS_PN(n_state=n_state, n_control=n_control, n_lam=n_lam, F_stiffness=F_stiffness)
    ...
    for n_trial in range(max_trial):
        ...
        print('------------------------------------------------')
        print('LCS generated!')
        print('n_state:', n_state, 'n_control:', n_control, 'n_lam:', n_lam)
        print('min_sig_lis', min_sig)
        trial_result['min_sig_list'] = min_sig

        # generate the training data
        training_data = lcs_learning.gen_data(lcs_mats, training_data_size, noise_level=1e-2)
        print('------------------------------------------------')
